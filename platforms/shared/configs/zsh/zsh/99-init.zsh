# Must load LAST (99- prefix). Two invariants depend on it: zsh-syntax-highlighting
...
#
# This runs LAST (after the plugin/mise inits above) on purpose. Three things
# reorder PATH after ~/.zshenv and would otherwise bury these dirs behind
# /usr/bin:
...
#
# So we prepend from a precmd hook too. This block runs after the mise init
# above, so our hook is appended to precmd_functions AFTER mise's and runs after
# it each prompt -- the genuine last word. `typeset -U path` (set in ~/.zshenv)
# makes the re-prepend idempotent: the dupe added earlier is dropped, floating
# gnubin back to the front. GNU sed/find/grep win too.
#
